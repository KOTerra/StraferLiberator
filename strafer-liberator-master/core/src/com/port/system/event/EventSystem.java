...
import com.port.entity.item.pickup.PickUp;
import com.port.entity.item.pickup.Pill;
import com.port.entity.mover.npc.friendly.Keanu;
import com.port.entity.mover.npc.friendly.Stonks;
import com.port.entity.mover.npc.friendly.Tsoukalos;
...
public class EventSystem extends Actor {
	...
	public void checkEvent() {
		if (!WorldData.PAUZA) {
			/*
			 * aici e practic cronologic povestea C: e cutscene T: e tutorial D: e dialog i:
			 * e item L: e locatie
			 */
			...
			//
			case 1: {
				if (worldListener.getWorldSection() == 11 && player.getWorldX() == playWorld.originalX
						&& player.getWorldY() == playWorld.originalY) {
					playWorld.initObject(new Tutorial("Cutscene", "wakeup", 3, false), WorldData.menuX,
							WorldData.menuY);
				}
				break;
			}
			...
			//
			case 4: {
				playWorld.initObject(new Tutorial("Mechanics", "inventory", 2, false), WorldData.menuX,
						WorldData.menuY);
				break;
			}
			//
			...
			case 11: {
				...
				if (worldListener.getWorldSection() == 11 && player.getWorldX() >= 90 * 64
						&& player.getWorldY() >= 60 * 64) {
					WorldData.nrEvent++;
				}
				break;
			}
			...
			case 13: {
				...
				if (worldListener.getWorldSection() == 12 && player.getWorldX() >= 40 * 64
						&& player.getWorldY() >= 100 * 64) {
					WorldData.nrEvent++;
				}
				break;
			}
			...
			case 16: {
				...
				if (worldListener.getWorldSection() == 22 && player.getWorldX() >= 64 * 64
						&& player.getWorldY() >= 3 * 64) {
					WorldData.nrEvent++;
				}
				break;
			}
			...
			case 19: {
				...
				if (worldListener.getWorldSection() == 23 && player.getWorldX() >= 38 * 64
						&& player.getWorldY() >= 98 * 64) {
					WorldData.nrEvent++;
