...
public class SchrodingersCat extends Goblin {
	...
	public SchrodingersCat(Scroller scrl, int x, int y) {
		...
		directie.put("D", StraferLiberator.assetManager.get("images/npc/inamic/schrodingersCat/schrodingersCat_m.gif",
				GifImage.class));
		directie.put("W", StraferLiberator.assetManager.get("images/npc/inamic/schrodingersCat/schrodingersCat_m.gif",
				GifImage.class));
		directie.put("A", StraferLiberator.assetManager.get("images/npc/inamic/schrodingersCat/schrodingersCat_m.gif",
				GifImage.class));
		directie.put("S", StraferLiberator.assetManager.get("images/npc/inamic/schrodingersCat/schrodingersCat_m.gif",
				GifImage.class));
		directie.put("idle", StraferLiberator.assetManager
				.get("images/npc/inamic/schrodingersCat/schrodingersCat_m_Idle.gif", GifImage.class));
	...
	private void checkAction() {
		if (!animationRunner.isActive() && metPlayer) {
			changedAnimation = false;
						...
						if (!players.isEmpty()) {
							...
							if (deltaPGX <= WorldData.WIDTH * 0.65f && deltaPGY <= WorldData.HEIGHT * 0.65f) {
								if (!WorldData.metSchrodingersCat && WorldData.nrEvent <= 22) {
									getWorld().addObject(new Tutorial("Combat", "SchrodingersCat", 3, false),
											WorldData.menuX, WorldData.menuY);
