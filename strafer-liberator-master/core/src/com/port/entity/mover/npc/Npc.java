...
public class Npc extends Movers {
	...
	protected boolean checkPlayerInChunck() {
		...
		if (player != null) {
			float dist = (player.getX() - this.getX()) * (player.getX() - this.getX())
					+ (player.getY() - this.getY()) * (player.getY() - this.getY());
			dist = (int) Math.sqrt(dist);

			if (dist <= WorldData.WIDTH) {
				return true;
			}
		}
		return false;
		...
		while (st <= dr) {
			...
			if (start.dist < searchDistanceMax) {
				for (int i = 0; i < 8; i++) {
					...
					if (mat[l + dx[i]][c + dy[i]] == false && l + dx[i] > 0 && c + dy[i] > 0 && l + dx[i] < 1000
							&& c + dy[i] < 1000) {
						// nu am mai fost aici
						// adaug in coada
						dr++;
	...
	protected void knockback(double timp, Actor attacker, double masa_attacker, double masa_this) {
		...
		xfinknockback = (int) (getX() + distance_added * Math.cos(grade_rezultanta)); // unde trebe sa ajunga dupa
																						// knockback
		yfinknockback = (int) (getY() + distance_added * Math.sin(grade_rezultanta));

		double fractiune = 60 / (timp * 1000);

		viteza_frame_x = sens_x * 2 * acceleratie_this * distance_added * fractiune;// *64; //distanta cu care e miscat
																					// pe fiecare frame pana in pozitia
																					// de knockback
